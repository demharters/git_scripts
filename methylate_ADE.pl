...
#
# usage:
#  ./methylate_ADE.pl RNA.pdb RES_NUM  > 6MeA-RNA.pdb
#
# This script methylates a single ADE residue with residue 
# number RES_NUM at position N6 
#
#
#
#
...
#               H51   H62                          H51
#                |    /                            |
#            H53-C5M-N6   H61- is replaced by H53-C5M-  
#                |   |                             |
#               H52  C6                            H52
#                  // \ 
#                 N1  C5--N7\\
...
# Copyright (c) 2014 Peter Minary
  ...
  $pdb_file_name = $ARGV[0];
  open(PDB_FILE,"$pdb_file_name");
  $res_num_target = $ARGV[1];

  # NN1 - CN2 type eq bond length in charmm27
  $r_bond_N6_C5M      =  1.366;               # in A
  
  $r_bond_C5M_H       =  1.11;                # in A
  $theta_N6_C5M_H     =  110.10;              # in deg
  $phi_C6_N6_C5M_H51  =     3.14*120.0/180.0; # in rad
  $phi_C6_N6_C5M_H52  = 2.0*3.14*120.0/180.0; # in rad 
  $phi_C6_N6_C5M_H53  = 3.0*3.14*120.0/180.0; # in rad
...
#NOUSE
  ...
  for ($iline=1;$iline<=$nline;$iline++) {
        ...
        if ( (substr($entry,0,3) eq 'TER') ||
         ...
         if ( (($resName[$iline] eq 'ADE') ||
               ($resName[$iline] eq 'A'  )) &&  
              ($resSeq_ADE != $resSeq[$iline]) &&
              ($res_num_target eq $resSeq[$iline])) {
             $resSeq_ADE = $resSeq[$iline];
             $natm_ADE  = 0;
             for ($jline=$iline;$jline<=$nline;$jline++) {
                  ...
                  if ($resSeq[$jline] eq  $resSeq_ADE) {
                      if ( substr($entry_jline,0,4) eq 'ATOM') { 
                           substr($entry_jline,17,3) = 'AME';
             ...
             # build C5M
               $ratio = $r_bond_N6_C5M/$r_bond_C5M_H;
               $xC5M = $xN6 + $ratio*($xH61 - $xN6);
               $yC5M = $yN6 + $ratio*($yH61 - $yN6);
               $zC5M = $zN6 + $ratio*($zH61 - $zN6);
               
             # build H51, H52 H53       

                 # use procedure applied within DC-REPSWA
                 # define r1=C6, r2=N6, r3=C5M, r, theta, phi
                 $r1x = $xC6; $r1y = $yC6; $r1z = $zC6;
                 $r2x = $xN6; $r2y = $yN6; $r2z = $zN6;
                 $r3x = $xC5M; $r3y = $yC5M; $r3z = $zC5M;

                 $r     = $r_bond_C5M_H;
                 $theta = $pi*(180.0-$theta_N6_C5M_H)/180.0;
               
                 # H51
                 $phi   = $phi_C6_N6_C5M_H51;
                 ...
                 # H53
                 $phi   = $phi_C6_N6_C5M_H53;
                 # build r4 based on r1, r2, r3, r, theta, phi
                 &build_r4;
                 # read out H53
                 $xH53 = $r4x; $yH53 = $r4y; $zH53 = $r4z;                    

                 $insert_line = $iline + $natm_ADE - 1;
         ...
         if ($insert_line == $iline) {
            ...
            if ($res_num_target eq $resSeq[$iline]) {
                if ((substr($entry,12,4) ne 'H61 ') &&
                   (substr($entry,12,4) ne ' H61')) {print "$entry\n";}
            } else {
              print "$entry\n";
            }
         }
