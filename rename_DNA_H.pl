#! /usr/bin/perl -w
#
# This script renames H-atoms added to DNA, so that it is 
# compatible with amber94/charmm27 topology
# The H atoms can be added using the pymol package for 
# example
#
# usage:
#       ./rename_DNA_H.pl A.pdb > A_.pdb
#
# A.pdb  :  pdb having some DNA with added H's
# A_.pdb :  DNA H's are renamed
#
...
# 2) rename H atoms of DNA residues                         #
...
#DEBUG
       ...
       #THY______________________________________________________
       if ( ($res_Name eq 'T')   ||
            ($res_Name eq 'THY') ||
            ($res_Name eq 'DT') ) {
...
#DEBUG
       ...
       #THY__________________________________________________END
