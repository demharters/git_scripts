#! /usr/bin/perl -w 
# 
# This script protonates all HIS residues that are missing atom HE2 
#
#     |                HE1
#  HN-N            __  /
#     |   HB1   ND1--CE1
...
# Bond(NE2-HE2)        =   Bond(NR1-H)           =  1.0 A   
# Bend(CD2-NE2-HE2)    =   Bend(CPH1-NR1-H)      =  125.5 deg 
# Tors(CG-CD2-NE2-HE2) =   Tors(CPH1-CPH1-NR1-H) =  180.0 deg     
#
#
#
  $r_bond_NE2_HE2     =  1.0;      # in A
  $theta_CD2_NE2_HE2  =  125.5;     # in deg
  $phi_CG_CD2_NE2_HE2 =  180.0;    # in deg
  $pi               =  3.14;     # in rad
...
#NOUSE
  ...
  for ($iline=1;$iline<=$nline;$iline++) {
        ...
        if ( (substr($entry,0,3) eq 'TER') ||
         ...
         if ( ($resName[$iline] eq 'HIS') &&  
              ($resSeq_HIS != $resSeq[$iline]) ) {
             $resSeq_HIS = $resSeq[$iline];
             $natm_HIS  = 0;
             $HE2_found = 0;
             for ($jline=$iline;$jline<=$nline;$jline++) {
                  if ($resSeq[$jline] eq  $resSeq_HIS) {
                      $natm_HIS++;
                      if ($atom[$jline] eq 'HE2') {$HE2_found=1;}
                      if ($atom[$jline] eq 'CG') {
                          $xCG = $x[$jline];
                          $yCG = $y[$jline];
                          $zCG = $z[$jline];
                      }
                      if ($atom[$jline] eq 'CD2') {
                          $xCD2 = $x[$jline];
                          $yCD2 = $y[$jline];
                          $zCD2 = $z[$jline];
                      }
                      if ($atom[$jline] eq 'NE2') {
                          $xNE2 = $x[$jline];
                          $yNE2 = $y[$jline];
                          $zNE2 = $z[$jline];
                      }
             ...
             if ($HE2_found==0) {
#DEBUG
#                 print "natm_HIS: $natm_HIS\n";
#DEBUG
                 # use procedure applied within DC-REPSWA

                 # define r1=CG, r2=CD2, r3=NE2, r, theta, phi
                 $r1x = $xCG; $r1y = $yCG; $r1z = $zCG;
                 $r2x = $xCD2; $r2y = $yCD2; $r2z = $zCD2;
                 $r3x = $xNE2; $r3y = $yNE2; $r3z = $zNE2;
                 
                 $r     = $r_bond_NE2_HE2;
                 $theta = $pi*(180.0-$theta_CD2_NE2_HE2)/180.0;
                 $phi   = $pi*$phi_CG_CD2_NE2_HE2/180.0;

                 # build r4 based on r1, r2, r3, r, theta, phi
                 &build_r4;

                 # read out HE2
                 $xHE2 = $r4x; $yHE2 = $r4y; $zHE2 = $r4z;                    

                 $insert_line = $iline + $natm_HIS - 1;
             }
	 }
...
#             print " zHE2 : $zHE2\n";
#DEBUG
             $x_field = sprintf("%8.5g",$xHE2);
             $y_field = sprintf("%8.5g",$yHE2);
             $z_field = sprintf("%8.5g",$zHE2);
